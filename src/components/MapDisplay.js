...
class MapDisplay extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      dim: {},   // { width: , hight: }
      appState: INACTIVE,
      msgIndex: Math.floor(Math.random() * Math.floor(msgs.length))
    };
...
*/
  ...
  renderLoading() {
    ...
    */
    ...
    const msg = (!msgs || msgs.length === 0 || !msgs[this.state.msgIndex]) ?
      "" : msgs[this.state.msgIndex];
      ...
      <View style={{ flex: 1 }}>
        <View
          ...
          <Text style=
            ...
            (שימו לב שהנתונים זמינים רק כשאתם בישראל)
          </Text>
          <Text style=
            {{ fontSize: 28, fontWeight: 'bold', textAlign: 'center' }}>
            {' '}
